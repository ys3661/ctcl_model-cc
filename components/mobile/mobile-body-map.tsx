...
interface MobileBodyMapProps {
  patientId: string
  editable?: boolean
}

export function MobileBodyMap({ patientId, editable = false }: MobileBodyMapProps) {
  ...
  const getLesionSize = (size: number) => {
    // Convert size in cm to pixel size for display, adjusted for zoom
    const baseSize = 16
    return Math.max(baseSize, size * 5) * (zoom / 100)
  }
      ...
      {lesionMappings.length === 0 ? (
        ...
        <>
          <div className="flex justify-between items-center">
            <div className="flex overflow-x-auto pb-2 gap-2 max-w-[70%]">
          ...
          {selectedMapping && (
            <>
              ...
              <Card className="overflow-hidden">
                <div
                  className="relative w-full aspect-[3/4] overflow-auto bg-slate-50"
                  style={{
                    maxHeight: "60vh",
